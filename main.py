...
if __name__ == "__main__":
    # Get the correct filename location / Url
    filename = get_valid_filename()
    # Choose the format of the dataset
    df = dataset_format(filename)
    print(df.columns)
    # Remove/ Drop the columns that the user does not want
    column_remove()
    print(df)
    # Remove/ Drop the chosen duplicates row
    drop_dups()
    # Fill null values for each numeric column, 1. remove 2. Mean 3. Medium
    null_values(df)
    # Remove the outliners in the columns chosen by user
    outliers_remove(df)
    # Export the dataframe to the format that the user desires
